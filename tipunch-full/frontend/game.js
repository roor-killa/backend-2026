...
const API_BASE = 'http://127.0.0.1:8016';
let apiOnline = false;
const LOCAL_CLIENTS = CLIENTS.map(client => ({ ...client }));
...
async function loadApiData() {
  try {
    ...
    if (!response.ok) throw new Error('API clients HTTP ' + response.status);
    const data = await response.json();
    if (!Array.isArray(data.clients) || data.clients.length === 0) return;

    CLIENTS = data.clients.map(apiClientToGameClient);
    apiOnline = true;
  } catch(e) {
    apiOnline = false;
    CLIENTS = LOCAL_CLIENTS.map(client => ({ ...client }));
  }
}

function apiClientToGameClient(apiClient) {
  const local = LOCAL_CLIENTS.find(client => client.id === apiClient.id)
    || LOCAL_CLIENTS.find(client => client.name === apiClient.name)
    || LOCAL_CLIENTS.find(client => client.cocktail === apiClient.cocktail)
    || LOCAL_CLIENTS[0];

  return {
    id: apiClient.id,
    name: apiClient.name || local.name,
    avatar: local.avatar,
    bg: local.bg,
    cocktail: apiClient.cocktail || local.cocktail,
    diff: apiClient.difficulty || local.diff,
    dialogue: apiClient.dialogue ? '"' + apiClient.dialogue + '"' : local.dialogue,
    recipe: apiClient.recipes ? apiClient.recipes[apiClient.cocktail] : null,
  };
}

function ingredientKey(ingredient) {
  return ingredient.key || INGREDIENT_KEYS[ingredient.n] || ingredient.n.toLowerCase().replace(/\s+/g, '_');
}

function cocktailForClient(client) {
  const base = COCKTAILS[client.cocktail];
  if (!base || !client.recipe || !client.recipe.target) return base;

  const target = client.recipe.target;
  return {
    ...base,
    tol: client.recipe.tolerance ?? base.tol,
    ingrs: base.ingrs.map(ingredient => {
      const key = ingredientKey(ingredient);
      return {
        ...ingredient,
        key,
        cl: target[key] ?? ingredient.cl,
      };
    }),
  };
}
...
function loadClient() {
  ...
  state.currentCocktail = cocktailForClient(client);
  state.dosages = [0, 0, 0];
  state.served = false;
...
async function serve() {
  ...
  try {
    result = apiOnline ? await scoreWithApi(ck) : null;
  } catch(e) {
    apiOnline = false;
  }

  if (!result) result = scoreLocally(ck);

  state.totalScore += result.score;
  showFeedback(result.score, ck, result.diffs);
}

function scoreLocally(ck) {
...
async function scoreWithApi(ck) {
  const keys = ck.ingrs.map(ingredientKey);
  const dosages = {};
  ck.ingrs.forEach((ingredient, index) => {
    dosages[keys[index]] = state.dosages[index];
  });
  ...
  const response = await fetch(API_BASE + '/score', {
    ...
    body: JSON.stringify({
      ...
      cocktail: state.currentClient.cocktail,
      dosages,
    }),
  });
  ...
  return {
    score: data.score,
    diffs: keys.map(key => data.diffs[key] ?? 0),
  };
}
