...
class PotomitanScraper(BaseScraper):
    ...
    def to_document(self, item: dict[str, Any]) -> dict[str, Any]:
        """Normalise une entrée Potomitan vers le schéma commun `documents`."""
        content_parts: list[str] = []
        if item.get("titre_fr"):
            content_parts.append(f"Titre français : {item['titre_fr']}.")
        if item.get("texte_creole"):
            content_parts.append(item["texte_creole"])
        if item.get("auteur"):
            content_parts.append(f"Auteur : {item['auteur']}.")
        if item.get("categorie"):
            content_parts.append(f"Catégorie : {item['categorie']}.")

        title = item.get("titre") or item.get("titre_fr") or "Entrée Potomitan"
        ...
        return {
            ...
            "metadata": {
                "source_name": item.get("source", "potomitan.info"),
