...
class PawoloTekScraper(BaseScraper):
    ...
    def to_document(self, item: dict[str, Any]) -> dict[str, Any]:
        """Normalise une entrée Pawolotek vers le schéma commun `documents`."""
        content_parts: list[str] = []
        if item.get("texte_creole"):
            content_parts.append(item["texte_creole"])
        if item.get("audio_url"):
            content_parts.append(f"Audio : {item['audio_url']}.")
        if item.get("hashtags"):
            content_parts.append(f"Hashtags : {' '.join(item['hashtags'])}.")
